...
def piechart():
    ...
    labelDF = pd.read_csv('ResumeClassificationDF.csv')
    le = LabelEncoder()
    labelDF['Label'] = le.fit_transform(labelDF['Label'])
    sizes = []
    sizes.append(count_nums(labelDF['Label'], 0))
    sizes.append(count_nums(labelDF['Label'], 1))   
    sizes.append(count_nums(labelDF['Label'], 2))   
    sizes.append(count_nums(labelDF['Label'], 3)) 

    labels = ['Peoplesoft resumes','React resumes','SQL Developer Lightning insight','workday resumes']
    values = sizes
...
def extract_info(text, filename):
    ...
    if filename.lower().endswith('.zip'):
        ...
        skills = joblib.load('BhuSkills.pkl','r')
        st.header('Skills')
        st.write(skills)
